...
void ShowHud()
{
    *(uint8_t*)(g_libGTASA + 0x7165E8) = 0; // eski HUD o‘chadi

    if(!pGame || !pNetGame) return;
    auto* ped = GamePool_FindPlayerPed();
    if(!ped) return;

    int slot = ped->byteCurWeaponSlot;

    int weaponid = 0, ammo = 0, ammoinclip = 0;
    if(slot >= 0 && slot < 13) {
        weaponid   = ped->WeaponSlots[slot].dwType;
        ammo       = ped->WeaponSlots[slot].dwAmmo;
        ammoinclip = ped->WeaponSlots[slot].dwAmmoInClip;
    }
...
{	
	...
	*(char**)(g_libGTASA + 0x5D1608) = "/storage/emulated/0/GoldenRP/files/";
}
